...
var medicinesList=[];
 ...
 editMedicines (context) async{
   var docid;
    
    CollectionReference medicineref = FirebaseFirestore.instance.collection("medicines");
    await medicineref.where("userid", isEqualTo:FirebaseAuth.instance.currentUser.uid).get().then((value) {
      value.docs.forEach((element) { 
          docid=element.id;
      });
    });          
    await medicineref.doc(docid).update({
...
editDiseases (context) async{
  var docid;
    
    CollectionReference diseasesref = FirebaseFirestore.instance.collection("diseases");
    await diseasesref.where("userid", isEqualTo:FirebaseAuth.instance.currentUser.uid).get().then((value) {
      value.docs.forEach((element) { 
          docid=element.id;
      });
    });

 await diseasesref.doc(docid).update({
        "userid": FirebaseAuth.instance.currentUser.uid,
  ...
  editUserInfo1(context) async{
    var docid;
    CollectionReference userref = FirebaseFirestore.instance.collection('users');
    await userref.where("userid", isEqualTo:FirebaseAuth.instance.currentUser.uid).get().then((value) {
      value.docs.forEach((element) { 
          docid=element.id;
      });
    });
...
class Edit1 extends StatelessWidget {
  @override
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async{
           m_Diabetes = controller1.text;
            ...
            //else if(chronicDiseaseList.isEmpty){AwesomeDialog(context: context,title: "Error" , body: Text("There is field is empty Please Fill it"))..show();}
            else{
              await editUserInfo1(context);
               await editMedicines(context);
             await editDiseases(context);
              chronicDiseaseList.clear();
               medicinesList.clear();
               Navigator.of(context).push(MaterialPageRoute(builder:(context)=>Edit2()));
            }
